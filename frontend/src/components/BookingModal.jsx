...
const BookingModal = ({ isOpen, onClose, preSelectedService }) => {
  ...
  const fetchAvailability = async () => {
    try {
      ...
      const response = await fetch('https://dyh6i3cqyxe3.manus.space/api/availability')
      if (response.ok) {
  ...
  const checkClientVerification = async (email) => {
    try {
      const response = await fetch('https://dyh6i3cqyxe3.manus.space/api/verification/check-client', {
        method: 'POST',
        headers: {
  ...
  const createVerificationSession = async () => {
    ...
    try {
      const response = await fetch('https://dyh6i3cqyxe3.manus.space/api/verification/create-session', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First check if client needs verification (only for studio bookings)
      if (selectedService === 'studio-access') {
        const clientStatus = await checkClientVerification(formData.email)
        
        if (clientStatus && clientStatus.needs_verification) {
          // Client needs verification - show verification step
      ...
      let endpoint = 'https://dyh6i3cqyxe3.manus.space/api/bookings'
      let data = {
        name: formData.name,
        email: formData.email,
